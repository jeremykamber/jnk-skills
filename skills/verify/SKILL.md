---
name: verify
description: Verify a completed change with evidence, honestly. User-invoked only via /skill:verify. Runs the narrowest verification that gives confidence, logs squawks, and reconciles the IOU ledger.
...
6. **Reconcile the IOUs.** Which unknowns from /skill:understand got answered? Remaining ones become squawks or next steps.
